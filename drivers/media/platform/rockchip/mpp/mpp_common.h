...
/*
 * Stubs for Rockchip vendor-kernel APIs not present in mainline.
 * These are no-ops; the hardware still functions but without these
 * vendor-specific power/IOMMU/QoS optimizations.
 */
...
static inline int  rockchip_iommu_disable(struct device *dev) { return 0; }
...
static inline void rockchip_save_qos(struct device *dev) {}
static inline void rockchip_restore_qos(struct device *dev) {}
static inline int  rockchip_pmu_idle_request(struct device *dev, bool idle)
{
	if (idle)
		return rockchip_pmu_block();
	rockchip_pmu_unblock();
	return 0;
}
static inline int  rockchip_iommu_force_reset(struct device *dev) { return 0; }
