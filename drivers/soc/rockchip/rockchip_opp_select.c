...
/* Vendor-only PLL scaling ops not present in mainline */
static inline int rockchip_pll_clk_rate_to_scale(struct clk *clk, unsigned long rate)
{
	return -ENODEV;
}

static inline int rockchip_pll_clk_adaptive_scaling(struct clk *clk, int scale)
{
	return -ENODEV;
}

static inline long rockchip_pll_clk_scale_to_rate(struct clk *clk, int scale)
{
	return -ENODEV;
}

/* Vendor PVTPLL soft-adjust not present in mainline */
static inline int rockchip_pvtpll_volt_sel_adjust(unsigned int clk_id, int volt_sel)
{
	return -ENODEV;
}
